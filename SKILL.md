...
## 場景判斷（觸發後第一件事：判斷現在是哪種情境）

| 同事說的話 | 走哪個流程 |
|-----------|-----------|
| 「我要開始用 ARTOGO 設計」「初始化」「第一次用」「設定設計工具」 | 走 **A · 首次設定流程** |
...
| 不確定 | 直接問同事：「你是想 (1) 設定環境 (2) 看用法 (3) 修問題 (4) 做設計？」 |
...
> 「歡迎！我是 ARTOGO Design Master ✨。看起來這是你第一次使用，我會幫你完整設定，全程大概 2 分鐘，你只要看著就好。可以開始了嗎？」
